...
void main(void) {
    ...
    // Set P2.1 as output (manual PWM)
    P2DIR |= BIT1;
    P2OUT &= ~BIT1;

    while (1) {
        int angle;
        for (angle = 0; angle <= 180; angle += 5) {  // Smaller step for smoother movement
            setServoAngle(angle);
            delay_ms(500);  // Slow down motion
        }
...
// Send repeated pulses to hold position temporarily
void setServoAngle(int angle) {
    if (angle < 0) angle = 0;
